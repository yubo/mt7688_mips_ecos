...
externC char* cyg_thread_get_name( cyg_handle_t thread) __THROW
{
    return ((Cyg_Thread *)thread)->get_name();
}
...
cyg_bool cyg_interrupt_bISR(void) __THROW
{
    return Cyg_Interrupt::ISR_processing();
}

cyg_bool cyg_interrupt_bDSR(void) __THROW
{
    return Cyg_Interrupt::DSR_processing();
}
...
externC cyg_count32 cyg_mbox_peek_size(cyg_handle_t mbox) __THROW
{
    return ((Cyg_Mbox *)mbox)->peek_size();
}

externC cyg_count32 cyg_mbox_peek_base(cyg_handle_t mbox) __THROW
{
    return ((Cyg_Mbox *)mbox)->peek_base();
}

externC void ** cyg_mbox_peek_itemqueue(cyg_handle_t mbox) __THROW
{
    return ((Cyg_Mbox *)mbox)->peek_itemqueue();
}
...
/*
externC char* cyg_mutex_threadqueue_highpri_name( cyg_mutex_t *mutex ) __THROW
{
    return ((Cyg_Thread *)(((Cyg_Mutex *)mutex)->queue.highpri()))->get_name();
}

externC cyg_bool_t cyg_mutex_threadqueue_empty( cyg_mutex_t *mutex ) __THROW
{
    return ((Cyg_Mutex *)mutex)->queue.empty();
}*/

externC char* cyg_mutex_threadqueue_highpri_name( cyg_mutex_t *mutex ) __THROW
{
    return ((Cyg_Mutex *)mutex)->queue_highpri_thread_name();
}

externC cyg_bool_t cyg_mutex_threadqueue_empty( cyg_mutex_t *mutex ) __THROW
{
    return ((Cyg_Mutex *)mutex)->queue_empty();
}


/*
externC cyg_handle_t cyg_threadqueue_highpri( cyg_threadqueue *threadqueue ) __THROW
{
    return (cyg_handle_t)((Cyg_ThreadQueue *)(*threadqueue))->highpri();
}

externC cyg_bool_t cyg_threadqueue_empty( cyg_threadqueue *threadqueue ) __THROW
{
    return ((Cyg_ThreadQueue *)(*threadqueue))->empty();
}*/
