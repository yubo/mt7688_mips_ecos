		...
		*(((UINT32 *)txd_1)) = SWAP32(*(((UINT32 *)txd_1)));
        ...
        //{
            ...
            MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF,
                ("TMR Report: ir = %d, TOA_high = %x, TOA_low = %x, TOD_high = %x, TOD_low = %x, delta_high = %x, delta_low_hex = 0x%x, delta_low = %lu\n",
...
Label_Legacy_PS:
		...
		else
		{
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ps> mgmt to legacy ps queue... (%d)\n", FlgIsDeltsFrame));
...
//			__FUNCTION__, pSrcBufVA, pHeader_802_11, tmac_info, tx_hw_hdr_len));
	...
	*/
    ...
    if ((pHeader_802_11->FC.Type == FC_TYPE_MGMT) &&
            (pHeader_802_11->FC.SubType == SUBTYPE_ACTION))
    {
        FRAME_TMR_UNPROTECTED_ACTION *pFrameTmr = (FRAME_TMR_UNPROTECTED_ACTION *)pHeader_802_11;
        if (pFrameTmr->Category == CATEGORY_UNPROTECTED_WNM)
        {
            mac_info.IsTmr = TRUE;
        }
    }
