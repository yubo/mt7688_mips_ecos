	...
	IN UUID_BSSID_CH_INFO	ApUuidBssid[])
{
	...
	UUID_BSSID_CH_INFO	ApUuidBssidTemp;
	ADD_HTINFO	RootApHtInfo, ApHtInfo;
	UCHAR	channel = pAd->CommonCfg.Channel;
	UCHAR RootApChannel, RootApCentralChannel;
...
if (CurOpMode == AP_MODE)
	...
	for (i = 0; i < pAd->ScanTab.BssNr; i++) 
	{
		...
		RootApCentralChannel = pAd->ScanTab.BssEntry[i].CentralChannel;
		RootApHtInfo = pAd->ScanTab.BssEntry[i].AddHtInfo.AddHtInfo;
...
VOID WscWriteConfToDatFile(RTMP_ADAPTER *pAd, UCHAR CurOpMode)
{
	...
	IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
	{
		for (index = 0; index < pAd->ApCfg.BssidNum; index++)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("pAd->ApCfg.MBSSID[%d].wdev.DefaultKeyId=%d\n", index, pAd->ApCfg.MBSSID[index].wdev.DefaultKeyId+1));
			if (index == 0)
