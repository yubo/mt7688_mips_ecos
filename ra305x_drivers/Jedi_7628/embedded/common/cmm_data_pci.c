	...
	OUT USHORT *FreeNumber)
{
	...
	if(pTxD->SDLen0 > 1568) 
		panic("RtmpPCI_WriteSingleTxResource <1>\n");
	if(pTxD->SDLen1 > 1568) 
		panic("RtmpPCI_WriteSingleTxResource <2>\n");	
	if((pTxD->SDLen0+pTxD->SDLen1)!= ((*((UINT32 *)pDMAHeaderBufVA))&0xffff))
		panic("RtmpPCI_WriteSingleTxResource <3>\n");
	if(txd_1->ft == TMI_FT_SHORT)
		panic("RtmpPCI_WriteSingleTxResource <4>\n");	
#endif
	...
	OUT	USHORT *FreeNumber)
{
	...
	if(pTxD->SDLen0 > 1568) 
		panic("RtmpPCI_WriteMultiTxResource <1>\n");
	if(pTxD->SDLen1 > 1568) 
		panic("RtmpPCI_WriteMultiTxResource <2>\n");	
	if((pTxD->SDLen0+pTxD->SDLen1)!= ((*((UINT32 *)pDMAHeaderBufVA))&0xffff))
		panic("RtmpPCI_WriteMultiTxResource <3>\n");
	if(txd_1->ft == TMI_FT_SHORT)
		panic("RtmpPCI_WriteMultiTxResource <4>\n");	
#endif
	...
	OUT	USHORT *FreeNumber)
{
	...
	if(pTxD->SDLen0 > 1568) 
		panic("RtmpPCI_WriteFragTxResource <1>\n");
	if(pTxD->SDLen1 > 1568) 
		panic("RtmpPCI_WriteFragTxResource <2>\n");	
	if((pTxD->SDLen0+pTxD->SDLen1)!= ((*((UINT32 *)pDMAHeaderBufVA))&0xffff))
		panic("RtmpPCI_WriteFragTxResource <3>\n");
	if(txd_1->ft == TMI_FT_SHORT)
		panic("RtmpPCI_WriteFragTxResource <4>\n");
...
/*			pTxD->DMADONE = 0; */
			...
			else if (!(pAd->ATECtrl.Mode & ATE_TXFRAME))
			{
				...
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("not complete sending yet, but someone pressed the Stop TX bottom\n"));
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO,("pAd->ate.Mode = 0x%02x\n", pAd->ATECtrl.Mode));
			}
			else
			{
...
/*		pTxD->DMADONE = 0; */
		...
		if(pTxD->SDPtr0 && pTxD->SDPtr1) 
		{
			...
			if(pTxD->SDLen0 >1568)
				panic("RTMPFreeTXDUponTxDmaDone <1>\n");
			if(pTxD->SDLen1 >1568)
				panic("RTMPFreeTXDUponTxDmaDone <2>\n");
			if((pTxD->SDLen0 + pTxD->SDLen1)!=((*((UINT32 *)pDMAHeaderBufVA))&0xffff))
				panic("RTMPFreeTXDUponTxDmaDone <3>\n");
		}
		else if(pTxD->SDPtr0)
		{
			if(pTxD->SDLen0 >1568)
			...
			panic("RTMPFreeTXDUponTxDmaDone <6>\n");
#endif
...
/*	int 		 i;*/
	...
	while (pMgmtRing->TxSwFreeIdx!= pMgmtRing->TxDmaIdx)
	{
		...
		if(pTxD->SDPtr0 && pTxD->SDPtr1) 
		{
			...
			if(pTxD->SDLen0 >1568)
				panic("RTMPHandleMgmtRingDmaDoneInterrupt <1>\n");
			if(pTxD->SDLen1 >1568)
				panic("RTMPHandleMgmtRingDmaDoneInterrupt <2>\n");
			if((pTxD->SDLen0 + pTxD->SDLen1)!=((*((UINT32 *)pDMAHeaderBufVA))&0xffff))
				panic("RTMPHandleMgmtRingDmaDoneInterrupt <3>\n");
		}
		else if(pTxD->SDPtr0)
		{
			if(pTxD->SDLen0 >1568)
			...
			panic("RTMPHandleMgmtRingDmaDoneInterrupt <6>\n");
#endif
...
/* management frame doesn't need encryption. so use RESERVED_WCID no matter u are sending to specific wcid or not */
	...
	if(pTxD->SDLen0 > 1568) 
		panic("MlmeHardTransmitTxRing <1>\n");
	if(pTxD->SDLen0 != ((*((UINT32 *)frm_buf))&0xffff))
	...
	while (pCtrlRing->TxSwFreeIdx!= pCtrlRing->TxDmaIdx)
	{
		...
		if(pTxD->SDPtr0)
			if(pTxD->SDLen0 >(4096-64))
				panic("RTMPHandleTxRing8DmaDoneInterrupt <1>\n");
			
		if(pTxD->SDPtr1)
			panic("RTMPHandleTxRing8DmaDoneInterrupt <2>\n");
		
		if(pTxD->SDPtr0 == NULL)
			panic("RTMPHandleTxRing8DmaDoneInterrupt <3>\n");		
#endif
#endif
	...
	while (pBcnRing->TxSwFreeIdx!= pBcnRing->TxDmaIdx)
	{
		...
		pTxD = (TXD_STRUC *) (pBcnRing->Cell[pBcnRing->TxSwFreeIdx].AllocVa);
				...
				if(pTxD->SDPtr0 && pTxD->SDPtr1) 
				{
					...
					if(pTxD->SDLen0 >1568)
						panic("RTMPHandleBcnDmaDoneInterrupt <1>\n");
					if(pTxD->SDLen1 >1568)
						panic("RTMPHandleBcnDmaDoneInterrupt <2>\n");
					if((pTxD->SDLen0 + pTxD->SDLen1)!=((*((UINT32 *)pDMAHeaderBufVA))&0xffff))
						panic("RTMPHandleBcnDmaDoneInterrupt <3>\n");
				}
				else if(pTxD->SDPtr0)
				{
					if(pTxD->SDLen0 >1568)
					...
					panic("RTMPHandleBcnDmaDoneInterrupt <6>\n");
#endif
