	...
	else
	{
		if (tr_entry->bEospNullSnd)
		{
			...
			*/
			...
			RTMPDeQueuePacket(ad, TRUE, AcQueId, pEntry->wcid, 1);//comment:this will use in context.
#ifdef UAPSD_DEBUG
...
static VOID ExtEventTmrCalResultHandler(RTMP_ADAPTER *pAd, UINT8 *Data, UINT32 Length)
{
	...
	ucFactorD = pTmrCalResult->ucTMRPhase;

	if (ucFactorD >= 6)
	{
	        u4FactorResult = u4FactorA +
				u4FactorB +
				((u4FactorC-4)*33/4) - 6400 +
				(8-(ucFactorD-6))*25;//unit is 0.25 nsec
	}
	else
	{
		u4FactorResult = u4FactorA +
                                u4FactorB +
                                ((u4FactorC-4)*33/4) - 6400 +
                                (6-ucFactorD)*25;//unit is 0.25 nsec
	}
        pAd->chipCap.TmrCalResult = u4FactorResult;
    }

    /* check if the TMR result is calibrated to negative value. */
    if (u4FactorB & 0x2000)
    {
        MTWF_LOG(DBG_CAT_ALL,
                DBG_SUBCAT_ALL,
                DBG_LVL_OFF,
                ("######## please reenable again! %s::u4TmrResult (0x%x)\n",
                __func__,
                u4FactorB
                ));
    }
    ...
    MTWF_LOG(DBG_CAT_ALL,
                DBG_SUBCAT_ALL,
                DBG_LVL_OFF,
                ("%s::FW LOG, A (0x%x), B (0x%x), C (0x%x), D (0x%x),TmrCalResult (0x%x)\n",
                __func__,
                u4FactorA,
                u4FactorB,
                u4FactorC,
...
static VOID CmdThemalSensorRsp(struct cmd_msg *msg, char *Data, UINT16 Len)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("ThemalSensor = 0x%x\n", EventExtCmdResult->u4SensorResult));
}
...
error:
 ...
 INT32 CmdSlotTimeSet(RTMP_ADAPTER *pAd, UINT8 SlotTime,UINT8 SifsTime,UINT8 RifsTime,UINT8 EifsTime)
 {
	struct cmd_msg *msg;
	INT32 ret=0;
