...
static void Radius_client_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, "RADIUS_CLIENT_RECEIVE : len= %d \n", len);
	if (len < 0)
	{
		perror("recv[RADIUS]");
		return;
	...
	if ((buf[0]!=0xff) && (RT_DEBUG_INFO <= DOT1X_RTDebugLevel))
	{
		int i;
                diag_printf("  r_dump %d bytes:",len);
...
/*Comment by rory
	...
	while(connect(sock, (struct sockaddr *) &serv, sizeof(serv)) < 0 && retry < 30)
	{
		cyg_thread_delay(1000);
		retry++;
		//DBGPRINT(RT_DEBUG_TRACE, "Connect to Radius Server(%s) fail retry(%d)\n", inet_ntoa(nserv->addr),retry);
	}
...
{
	rtapd->radius->auth_serv_sock = socket(AF_INET, SOCK_DGRAM, 0);

	if (rtapd->radius->auth_serv_sock < 0)
	{
		diag_printf("error:socket[PF_INET,SOCK_DGRAM]");
		return -1;
	}
