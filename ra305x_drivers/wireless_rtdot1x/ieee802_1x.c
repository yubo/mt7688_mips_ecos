...
static void ieee802_1x_send(rtapd *rtapd, struct sta_info *sta, u8 type, u8 *data, size_t datalen)
{
	...
	if (sta->ethertype == ETH_P_PRE_AUTH)
	{
		//if (send(sta->SockNum/*rtapd->eth_sock*/, buf, len, 0) < 0)
			...
			EcosSendToEth(CONFIG_DEFAULT_LAN_NAME,buf, len);
			EcosSendToEth(CONFIG_DEFAULT_WAN_NAME,buf, len);
	}
	else
	{
...
//	}
	...
	Dot1xhmac_md5(sta->eapol_key_sign, sta->eapol_key_sign_len, buf, sizeof(*hdr) + len, key->key_signature);
...
#endif //LINUX//
#ifdef __ECOS
	if (DOT1X_RTMPCompareMemory(buf, SNAP_802_1H, 6) == 0)
#endif
		buf += LENGTH_802_1_H;
	hdr = (struct ieee802_1x_hdr *) buf;
				...
				u8 *shared_secret, size_t shared_secret_len)
{
	...
	if(keys && keys->recv_len != 0)
	{
		...
		*/
		...
		DBGPRINT(RT_DEBUG_ERROR, "IEEE802_1x_Get_Keys, PMK_len = %d\n",keys->recv_len );
		DBGPRINT(RT_DEBUG_ERROR, "PMK = %x %x %x %x %x %x %x ...%x \n",\
			keys->recv[0],keys->recv[1],keys->recv[2],keys->recv[3],\
			keys->recv[4],keys->recv[5],keys->recv[6],keys->recv[15]);
		DBGPRINT(RT_DEBUG_ERROR, "PMK[16] = %x %x %x %x %x %x %x %x \n",\
