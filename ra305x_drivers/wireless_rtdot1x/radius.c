...
void Radius_msg_finish(struct radius_msg *msg, u8 *secret, size_t secret_len)
{
	if (secret)
	{
		...
		Dot1xhmac_md5(secret, secret_len, msg->buf, msg->buf_used, (u8 *) (attr + 1));
	}
	else
		msg->hdr->length = htons(msg->buf_used);
...
int Radius_msg_verify(struct radius_msg *msg, u8 *secret, size_t secret_len, struct radius_msg *sent_msg)
{
	...
	Dot1xhmac_md5(secret, secret_len, msg->buf, msg->buf_used, auth);
	...
	Dot1xMD5Init(&context);
	Dot1xMD5Update(&context, (u8 *) msg->hdr, 1 + 1 + 2);
	Dot1xMD5Update(&context, sent_msg->hdr->authenticator, MD5_MAC_LEN);
	Dot1xMD5Update(&context, (u8 *) (msg->hdr + 1), msg->buf_used - sizeof(*msg->hdr));
	Dot1xMD5Update(&context, secret, secret_len);
	Dot1xMD5Final(hash, &context);
			   ...
			   size_t secret_len, struct radius_msg *sent_msg)
{
	...
	Dot1xMD5Init(&context);
	Dot1xMD5Update(&context, msg->buf, 4);
	Dot1xMD5Update(&context, sent_msg->hdr->authenticator, MD5_MAC_LEN);
	if (msg->buf_used > sizeof(struct radius_hdr))
		Dot1xMD5Update(&context, msg->buf + sizeof(struct radius_hdr), msg->buf_used - sizeof(struct radius_hdr));
	Dot1xMD5Update(&context, secret, secret_len);
	Dot1xMD5Final(hash, &context);
	if (memcmp(hash, msg->hdr->authenticator, MD5_MAC_LEN) != 0)
	{
...
void Radius_msg_make_authenticator(struct radius_msg *msg, u8 *data, size_t len)
{
	...
	l = random();
	Dot1xMD5Init(&context);
	Dot1xMD5Update(&context, (u8 *) &tv, sizeof(tv));
	Dot1xMD5Update(&context, data, len);
	Dot1xMD5Update(&context, (u8 *) &l, sizeof(l));
	Dot1xMD5Final(msg->hdr->authenticator, &context);
}
			   ...
			   u8 *secret, size_t secret_len, size_t *reslen)
{
	...
	while (left > 0)
	{
		...
		if (first)
		{
			Dot1xMD5Update(&context, sent_msg->hdr->authenticator,
				  MD5_MAC_LEN);
			Dot1xMD5Update(&context, key, 2); /* Salt */
			first = 0;
		} else
			Dot1xMD5Update(&context, pos - MD5_MAC_LEN, MD5_MAC_LEN);
		Dot1xMD5Final(hash, &context);
				  ...
				  u8 *data, size_t data_len, u8 *secret, size_t secret_len)
{
	...
	Dot1xMD5Init(&context);
	Dot1xMD5Update(&context, secret, secret_len);
	Dot1xMD5Update(&context, msg->hdr->authenticator, 16);
	Dot1xMD5Final(hash, &context);
	...
	while (pos < buf_len)
	{
		Dot1xMD5Init(&context);
		Dot1xMD5Update(&context, secret, secret_len);
		Dot1xMD5Update(&context, &buf[pos - 16], 16);
		Dot1xMD5Final(hash, &context);
