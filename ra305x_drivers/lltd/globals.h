...
//#define __DEBUG__
//#define BYTE_DUMP
//#define ADJUSTING_FOR_EARLY_WAKEUP
//#define SIOCGSTAMP_NOT_AVAILABLE
#define die LLTD_DIE
...
#define OID_802_DOT1X_LLTD_DATA			0x0680
#define RT_OID_802_DOT1X_LLTD_DATA		(OID_GET_SET_TOGGLE | OID_802_DOT1X_LLTD_DATA)
...
//add by woody
GLOBAL  etheraddr_t	g_hwaddr_eth0;
GLOBAL  etheraddr_t	g_hwaddr_eth1;
GLOBAL  etheraddr_t	g_hwaddr_ra0;
...
#define IS_ETH0(_a)	\
	(_a[0] == 'e' && _a[1] == 't' && _a[2] == 'h' && _a[3] == '0')
#endif //__ECOS//

int LLTP_RT_ioctl(int param, char  *data, int data_len, char *name,int flags);
unsigned char* LLTD_Ecos_MemPool_Alloc (unsigned long Length,int Type);
void LLTD_EcosSendToRaw(char* if_name, char* buf, size_t len);
void LLTD_FORWARD_EcosSendToRaw(char* if_name, char* buf, size_t len);

void Forwording_To_IF(struct ifnet *ifp, unsigned char *inpkt, int len);
struct eth_drv_sc* LLTD_FIND_SC(char* if_name);

#define SIOCIWFIRSTPRIV                                                  0x00
#define RT_PRIV_IOCTL								(SIOCIWFIRSTPRIV + 0x01)  
#define RTPRIV_IOCTL_SET							(SIOCIWFIRSTPRIV + 0x02)

#define RT_OID_802_11_BSSID				0x0713
typedef unsigned char			BOOLEAN;
typedef unsigned long			ULONG;
typedef void (*LLTD_TIMER_FUNCTION)(void *);


struct LltdEcosTimer
{
...
typedef struct  _LLTD_TIMER_STRUCT    {
	struct LltdEcosTimer		TimerObj;       // Ndis Timer object
...
}LLTD_TIMER_STRUCT, *PLLTD_TIMER_STRUCT;

typedef struct _LLTD_CMD_STRUCT
{
         struct ifnet *ifp;
	  unsigned char *inpkt;
	  int len;
        struct lltd_cmd_s *next;
} LLTD_CMD_STRUCT, *PLLTD_CMD_STRUCT;
