...
struct service_t *find_service(char *name)
{
	...
	diag_printf("\n[DDNS]:%s  :can't find server",__FUNCTION__);
	return NULL;
}
...
//  
//------------------------------------------------------------------------------
#if 0  //by ziqiang ,update ddns.oray.net 
...
int ORAY_update_entry(struct user_info *info)
{
	...
	if (ORAY_mode == 0 || ORAY_mode == 2)
	{
		...
		{
		case 0:
			DDNS_LOG("update %s with IP %s successfully\n", info->service->default_server, MY_IP_STR);
			if (ORAY_mode == 0)
				info->updated_time = GMTtime(0);
			
			ORAY_mode = 1;
			info->ddns_ticks = keepalive_ticks;
			return UPDATERES_OK;
			
		case -1:
			info->ddns_ticks = 60 * 100;
			return UPDATERES_ERROR;
...
int ORAY_update_entry(struct user_info *info)
{
	...
	len = sprintf(bp, "GET %s?", info->service->default_request);
	bp += len;
	...
	*bp = '\0';
	...
	case 200:
		//ip changed,and updata successful
		if(strstr(buf, "good") != NULL)
		{
			retval = UPDATERES_OK;
			DDNS_DBG("\n[DDNS]updata ip successful");
		}
		//ip not changed ,and we just not updata
		else if (strstr(buf,"nochg")  !=  NULL)
		{
			retval = UPDATERES_OK;
			DDNS_DBG("\n[DDNS]updata ip successful");
		}
		else if (strstr(buf,"nohost") !=NULL)
		{
			retval = UPDATERES_ERROR;
			DDNS_DBG("\n[DDNS]check if the hostname is correct");
		} else
		retval = UPDATERES_ERROR;
