...
#ifndef CONFIG_NET_USE_CMALLOC
#define NET_MEMPOOL_SIZE  roundup(CYGPKG_NET_MEMPOOL_SIZE,MSIZE)
#endif
extern	int config_net_mbuf_usage;
extern	int config_net_cluster_usage;

#define NET_MBUFS_SIZE		(config_net_mbuf_usage*1024)
#define NET_CLUSTERS_SIZE	(config_net_cluster_usage*1024)
...
extern unsigned char net_mbufs_area[];
...
extern unsigned char net_clusters_area[];
...
extern	int config_net_cluster_4k_usage;
#define NET_CLUSTERS_4K_SIZE	(config_net_cluster_4k_usage*1024)
extern unsigned char net_clusters_area_4k[];
static cyg_handle_t    net_clusters_4k;
static cyg_mempool_fix net_clusters_pool_4k;
int nmbclusters_4k = 0;
...
#ifdef	CONFIG_NET_CLUSTER_GROUP2
static struct net_stats stats_cluster_alloc_4k;
#endif
...
#ifdef CONFIG_NET_USE_CMALLOC
	res = malloc(sizeof(struct vm_zone));
#else
...
#ifdef	CONFIG_NET_CLUSTER_GROUP2	
    cyg_mempool_fix_create(&net_clusters_area_4k,
                           NET_CLUSTERS_4K_SIZE,
                           NET_CLUSTER_GROUP2_SIZE,
                           &net_clusters_4k,
                           &net_clusters_pool_4k);	
	mbutl_4k = (struct mbuf *)&net_clusters_area_4k;
#endif	

	mbutl = (struct mbuf *)&net_clusters_area;
//Eddy
//    mclrefcnt = net_clusters_refcnt;
    mclrefcnt = (char *)calloc(1, (NET_CLUSTERS_SIZE/MCLBYTES)+1);
    if (mclrefcnt == 0)
        panic("mclrefcnt");

    nmbclusters = NET_CLUSTERS_SIZE/MCLBYTES;
	
#ifdef	CONFIG_NET_CLUSTER_GROUP2	
    mclrefcnt_4k = (char *)calloc(1, (NET_CLUSTERS_4K_SIZE/NET_CLUSTER_GROUP2_SIZE)+1);
    if (mclrefcnt_4k == 0)
        panic("mclrefcnt_4k");

    nmbclusters_4k = NET_CLUSTERS_4K_SIZE/NET_CLUSTER_GROUP2_SIZE;	
#endif	
}
...
void cyg_kmem_print_stats( void )
{
    ...
    diag_printf( "   mbufs %ld, clusters %ld, free clusters %ld\n   Group2 pool clusters %ld, free clusters %ld\n",
                 ...
                 mbstat_4k.m_clusters,	/* clusters obtained from page pool */
                 /* mbstat.m_spare, */	/* spare field */
                 mbstat_4k.m_clfree	/* free clusters */ 
#else
	diag_printf( "	 mbufs %ld, clusters %ld, free clusters %ld\n",
				 mbstat.m_mbufs,	/* mbufs obtained from page pool */
				 mbstat.m_clusters, /* clusters obtained from page pool */
...
#ifdef	CONFIG_NET_CLUSTER_GROUP2	
    cyg_mempool_fix_get_info( net_clusters_4k, &info );
    diag_printf( "   Group2 pool: total %7d, free %7d, blocksize %4d\n",
                 info.totalmem,
                 info.freemem,
                 info.blocksize
        );	
...
#ifdef	CONFIG_NET_CLUSTER_GROUP2
int
cyg_mtocl_4k(u_long x)
{
    int res;
    res = (((u_long)(x) - (u_long)mbutl_4k) >> NET_CLUSTER_GROUP2_SIZE);
    return res;
}

struct mbuf *
cyg_cltom_4k(u_long x)
{
    struct mbuf *res;
    res = (struct mbuf *)((caddr_t)((u_long)mbutl_4k + ((u_long)(x) << NET_CLUSTER_GROUP2_SIZE)));
    return res;
}


void *
cyg_net_cluster_alloc_4k(void)
{
    void *res;

    START_STATS();
    log(LOG_MDEBUG, "Allocate cluster 4k = ");
    res = cyg_mempool_fix_try_alloc(net_clusters_4k);
    FINISH_STATS(stats_cluster_alloc_4k);
    log(LOG_MDEBUG, "%p\n", res);
    return res;
}
#endif
...
{
	if(!s){diag_printf("\nError:copyout source NULL\n");return -1;}
	if(!d){diag_printf("\nError:copyout dest NULL\n");return -1;}
    memcpy(d, s, len);
    return 0;
}
...
{
	if(!s){diag_printf("\nError:copyin source NULL\n");return -1;}
	if(!d){diag_printf("\nError:copyin dest NULL\n");return -1;}
    memcpy(d, s, len);
    return 0;
}
...
void
show_network_interface(pr_fun *pr, char * intf)
{
    int i, error;
    struct radix_node_head *rnh;
    struct ifnet *ifp;
	...
	if_indextoname(ifp->if_index, name, 64);
	if (name != NULL)	
		if ((intf == NULL) || (intf && (!strcmp(intf, name))))
			_show_ifp(ifp, pr);
    }
    cyg_scheduler_unlock();
}
