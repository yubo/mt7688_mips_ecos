...
static void print_title(FILE *fp)
{
	...
	time(&now);
	fprintf(fp, "/*\n");
	fprintf(fp, " * Generated by Ralink WEB converter: %s", ctime(&now));
...
static void print_common_head(FILE *fp)
{
	fprintf(fp, "#include <http_proc.h>\n");
...
/*dliu,031001 :add */
       ...
       #include <unistd.h>
		...
		fprintf(c1_outfp, "\n\nvoid web_%s(http_req *req, char *datap)\n{\n", basic_name);
		...
		if(process_inline_c) {
			...
			while (*last && ((next = strstr(last, "<%")) != NULL)) {
				...
				if(next > last){
					...
					//fprintf(c1_outfp, "\tWEB_write_blk(req, %s_s%d, idx, %s_s%d_len);\n\n", 
					//	basic_name, nsection, basic_name, nsection);
					fprintf(c1_outfp, "\tWEB_write_blk(req, datap, %d, %d);\n\n", 
						outbytes, (next-last));
					outbytes += (next-last);
					nsection++;
				}
			...
			//fprintf(c1_outfp, "\tWEB_write_blk(req, %s_s%d, idx, %s_s%d_len);\n\n", 
			//			basic_name, nsection, basic_name, nsection);
			fprintf(c1_outfp, "\tWEB_write_blk(req, datap, 0, %d);\n\n", 
						proc_blk_len);
			nsection++;
		}
...
int proc_cgi(char *outdir, FILE *outfp, char *basic_name, char *infile, int filelen)
{
	...
	*(newblk+newblk_len)='\0';
	...
	fprintf(cgifp, "extern char *cgi_%s(http_req *req);\n", basic_name);
	fclose(cgifp);

	fprintf(outfp, "\nchar *cgi_%s(http_req *req)\n{", basic_name);
