 ...
 * code.
 *
 * $Id: ip_ipsec_pxy.c,v 1.1 2010/01/28 11:09:57 dennis Exp $
 *
 */
#define	IPF_IPSEC_PROXY
#define	IPF_ESP_PROXY
...
struct esphdr
{
    unsigned long   spi;            /* Security Policy Index */
...
nat_t *nat;
{
	...
	if (dlen < 16)
		return -1;
	m_copydata(m, off, sizeof(cookies), cookies);
	//ipsec_buffer = (char *)m + off;
...
int ippr_esp_init(void)
{
	return 0;
}

int ippr_esp_new(fin, aps, nat)
fr_info_t *fin;
ap_session_t *aps;
nat_t *nat;
{
	...
	if (fin->fin_dlen < sizeof(struct esphdr))
		return -1;
		
	bcopy(fin->fin_dp, &esp, sizeof(struct esphdr));
	KMALLOCS(aps->aps_data, u_32_t *, sizeof(struct esp_pxy));
	memset(aps->aps_data, 0, sizeof(struct esp_pxy));
	aps->aps_psiz = sizeof(struct esp_pxy);
	
	espxy = (struct esp_pxy *)(aps->aps_data);
	espxy->spi[fin->fin_out] = esp.spi;
	return 0;
}

int ippr_esp_match(fin, aps, nat)
fr_info_t *fin;
ap_session_t *aps;
nat_t *nat;
{
	...
	if (fin->fin_dlen < sizeof(struct esphdr) || (aps->aps_data == NULL))
		return -1;
		
	bcopy(fin->fin_dp, &esp, sizeof(struct esphdr));
	espxy = (struct esp_pxy *)(aps->aps_data);
	
	if(espxy->spi[fin->fin_out] == 0)
	{
		espxy->spi[fin->fin_out] = esp.spi;
	}
	else 
	{
		if(espxy->spi[fin->fin_out] != esp.spi)
		{
			return -1;
		}
	}
