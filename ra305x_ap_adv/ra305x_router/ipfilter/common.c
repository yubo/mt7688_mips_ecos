 ...
 */
int	hostmask(seg, sa, msk, top, pp, cp, tp, linenum)
char	***seg;
u_32_t	*sa, *msk, *top;
u_short	*pp, *tp;
int	*cp;
...
char *getifname(void *ptr)
{
	return ((struct ifnet *)ptr)->if_xname;
}

void add_acttime(char ***cp, int *stime, int *etime)
{
	*stime = atoi(**cp);
	(*cp)++;
	*etime = atoi(**cp);
