...
int     linenum;
{
	...
	/* 01/12/03 Roger ++,  */	
	} else if (!strcasecmp("call", *cpp)) {
		if (!strcasecmp(*(cpp+1), "urlfilter")) {
			extern int urlfilter_match(int, ip_t *, fr_info_t *);
			fil.fr_func = urlfilter_match;
		}
		else if (!strcasecmp(*(cpp+1), "dnfilter")) {
			extern int dnfilter_match(int, ip_t *, fr_info_t *);
			fil.fr_func = dnfilter_match;
		}
		cpp++;
	/* 01/12/03 Roger --,  */	
	} else {
	...
	/* 16/02/04 Roger ++,  */	
	if (!strcasecmp("seclog", *cpp)) {
		if (!*++cpp) {
			fprintf(stderr, "%d: missing source specification\n",
				linenum);
			return NULL;
		}
		fil.fr_seclog = atoi(*cpp);
		cpp++;
		fil.fr_flags |= FR_SEC_LOG;
	}
	/* 16/02/04 Roger --,  */	
...
"%d: use proto ipv6-icmp with IPv6 (or use proto 1 if you really mean icmp)\n",
	...
	if (!strcasecmp(*cpp, "all")) {
		...
		if (hostmask(&cpp, (u_32_t *)&fil.fr_src,
			     (u_32_t *)&fil.fr_smsk, (u_32_t *)&fil.fr_tsrc, &fil.fr_sport, &ch,
			     &fil.fr_stop, linenum)) {
			return NULL;
		}
		...
		if (hostmask(&cpp, (u_32_t *)&fil.fr_dst,
			     (u_32_t *)&fil.fr_dmsk, (u_32_t *)&fil.fr_tdst, &fil.fr_dport, &ch,
			     &fil.fr_dtop, linenum)) {
			return NULL;
		}
		if ((ch != 0) && (fil.fr_proto != IPPROTO_TCP) &&
	...
	if (*cpp && !strcasecmp(*cpp, "act-time")) {
		...
		cpp++;
		add_acttime(&cpp, &(fil.fr_actst), &(fil.fr_actet));
		cpp++;
		
		if((curtime >= fil.fr_actst) && (curtime <= (fil.fr_actet?:86400)))
			fil.fr_flags &= ~FR_INACTIVE_SCHED;
		else
			fil.fr_flags |= FR_INACTIVE_SCHED;
	}
	/* 24/12/03 Roger --,  */	
