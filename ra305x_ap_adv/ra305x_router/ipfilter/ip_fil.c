...
extern void macfilter_check_acttime(void);
extern void urlfilter_check_acttime(void);
...
#endif
int	fr_running = 0;
unsigned int ipfr_slowtimer_ch;
 ...
 */
int IPL_EXTERN(ioctl)(dev, cmd, data, mode, p)
struct proc *p;
dev_t dev;
u_long cmd;
caddr_t data;
...
caddr_t data;
{
	...
	//	ftail = fprev = &ipauth;
	//else 
	if ((fp->fr_flags & FR_ACCOUNT) && (fp->fr_v == 4))
 ...
 */
int IPL_EXTERN(open)(dev, flags, devtype, p)
int devtype;
struct proc *p;
dev_t dev;
int flags;
...
int IPL_EXTERN(close)(dev, flags, devtype, p)
int devtype;
struct proc *p;
dev_t dev;
int flags;
...
# endif
		...
		if (dst == 0) {
			...
			{
				m_freem(m); /* 18/02/05 Roger,  free mbufs */	
				return -1;
			}
		} else
...
void fr_checkacttime(void)
{
	extern void flush_fcache(void);
	register struct frentry *fr;
	register ipnat_t *np;
	char daymap[7]={4,5,6,0,1,2,3};
	int curtime = GMTtime(0), act, week, day;
	week = curtime % 604800;
	day = week % 86400;
	week = week/86400;
	for (fr = ipfilter[1][fr_active]; fr; fr = fr->fr_next) {
		switch(fr->fr_actm){
			case 1:
				if(!(fr->fr_actd & (1<<daymap[week])))
				{
					if((fr->fr_flags & FR_INACTIVE_SCHED) == 0)
					{
						fr->fr_flags |= FR_INACTIVE_SCHED;
						flush_fcache();
					}
					continue;
				}
			case 2:
				if((day >= fr->fr_actst) && (day <= (fr->fr_actet?:86400)))
				{
	...
	for (np = nat_list; np; np = np->in_next) {
		switch(np->fr_actm){
			case 1:
				if(!(np->fr_actd & (1<<daymap[week])))
				{
					if((np->in_flags & IPN_INACTIVE_SCHED) == 0)
					{
						np->in_flags |= IPN_INACTIVE_SCHED;
					}
					continue;
				}
			case 2:
				if((day >= np->fr_actst) && (day <= (np->fr_actet?:86400)))
				{
