...
numeric_node_addr(struct addrinfo *ai, const char *node, 
   ...
   //     if (!inet_pton(AF_INET6, node, (void *)&sa->sin6_addr)) {
...
// convertion.
#define CYGNUM_NS_DNS_GETADDRINFO_ADDRESSES 1
static int
...
/*
* you must be careful of the res->ai_addrlen,if ai_family is AF_INET6,it may be 32,but the size 
* struct sockaddr_in6 is 28
*/
int   
            ...
            const struct addrinfo *hints, struct addrinfo **res)
{
    ...
    *res = ai = alloc_addrinfo(NULL);
    ...
    if (nodename) {
        ...
        //what if we don't set AI_NUMERICHOST flag??,am i right here?
        	if (  (err =numeric_node_addr(ai, nodename, hints, port) )!=EAI_NONE )
